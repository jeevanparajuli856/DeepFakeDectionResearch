...
def train_one_scenario(
    ...
    # filter data
    if unified:
        # unified training: all scenarios, real vs sd only
        tr = tr[tr.generator_family.isin(["real", "sd"])]
        va = va[va.generator_family.isin(["real", "sd"])]
        logger.info(
            f"[unified] Train samples: {len(tr)} | Val samples: {len(va)}"
        )
    else:
        ...
        logger.info(
            f"[{scenario}] Train samples: {len(tr)} | Val samples: {len(va)}"
        )
